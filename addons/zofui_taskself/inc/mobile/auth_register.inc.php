...
if($do == 'register') {
	if($_W['ispost'] && $_W['isajax']) {
		...
		if (!empty($fan) && !empty($fan['fanid'])) {
			pdo_update('mc_mapping_fans', array('uid'=>$data['uid']), array('fanid'=>$fan['fanid']));
		}
		if(_mc_login($data)) {
	...
	$settings = array(
		...
		'do' => 'auth_register',
		'title' => '注册',
		'regurl' => empty($this->module['config']['regurl']) ? '' : $this->module['config']['regurl'],
	);

	//template('auth/register');
	include $this->template('auth/register');
	exit;
}
