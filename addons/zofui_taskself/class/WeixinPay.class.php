...
class WeixinPay
{
	...
	//企业付款
	public function sendMoneyToUser($arr,$module,$no='') {
		global $_W;
		...
		$data['partner_trade_no'] = empty( $no ) ? $data['mchid'].date("Ymd",time()).date("His",time()).rand(1111,9999) : $no;		
	...
	public function sendhongbaoto($arr,$module,$no='') {

		$key = $module->module['config']['apikey'];
		$mchid = $module->module['config']['mchid'];
		
		$data['mch_id'] = $mchid;
		$data['mch_billno'] = empty( $no ) ? $data['mch_id'].date("Ymd",time()).date("His",time()).rand(1111,9999) : $no;
		$data['nonce_str'] = $this->createNoncestr();
	...
	public function wxHttpsRequestPem( $vars,$url, $second=30,$aHeader=array()) {
		...
		curl_setopt($ch,CURLOPT_SSLCERT,IA_ROOT.'/addons/zofui_taskself/cert/'.$_W['uniacid'].'/apiclient_cert.pem');
		curl_setopt($ch,CURLOPT_SSLKEYTYPE,'PEM');
		curl_setopt($ch,CURLOPT_SSLKEY,IA_ROOT.'/addons/zofui_taskself/cert/'.$_W['uniacid'].'/apiclient_key.pem');
		curl_setopt($ch,CURLOPT_CAINFO,'PEM');
		curl_setopt($ch,CURLOPT_CAINFO,IA_ROOT.'/addons/zofui_taskself/cert/'.$_W['uniacid'].'/rootca.pem');
		if( count($aHeader) >= 1 )
		{
